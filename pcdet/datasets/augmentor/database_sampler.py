...
class DataBaseSampler(object):
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, sampled_scores, mv_height=None, sampled_gt_boxes2d=None):
        score_keys = list(sampled_scores.keys())
        gt_boxes_mask = data_dict['gt_boxes_mask']
        gt_boxes = data_dict['gt_boxes'][gt_boxes_mask]
        gt_names = data_dict['gt_names'][gt_boxes_mask]
        gt_scores = {}
        for score_key in score_keys:
            gt_scores[score_key] = data_dict[score_key][gt_boxes_mask]
        ...
        sampled_obj_scores = {}
        for score_key in score_keys:
            sampled_obj_scores[score_key] = np.array([x[score_key] for x in total_valid_sampled_dict])
        ...
        for score_key in score_keys:
            data_dict[score_key] = np.concatenate([data_dict[score_key], sampled_obj_scores[score_key]], axis=0)
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                sampled_scores = {}
                for key in sampled_dict[0].keys():
                    if key == 'score':
                        continue
                    if 'score' in key:
                        sampled_scores[key] = np.array([x[key] for x in sampled_dict], dtype=np.float32)
        ...
        if total_valid_sampled_dict.__len__() > 0:
            ...
            data_dict = self.add_sampled_boxes_to_scene(
                data_dict, sampled_gt_boxes, total_valid_sampled_dict, sampled_scores, sampled_mv_height, sampled_gt_boxes2d
            )
