...
class DataAugmentor(object):
        ...
        for cur_cfg in aug_config_list:
            if not isinstance(augmentor_configs, list):
                if cur_cfg.NAME in augmentor_configs.DISABLE_AUG_LIST:
                    continue
                if cur_cfg.NAME == 'gt_sampling' and augmentor_configs.get('SPLIT_NAME', None) is not None:
                    for idx, db_info_path in enumerate(cur_cfg.DB_INFO_PATH):
                        cur_cfg.DB_INFO_PATH[idx] = db_info_path.split('.')[0] + '_' + augmentor_configs.SPLIT_NAME + '.pkl'
    ...
    def update_gt_sampling(self):
        for idx, cur_augmentor in enumerate(self.data_augmentor_queue):
            if isinstance(cur_augmentor, database_sampler.DataBaseSampler):
                self.data_augmentor_queue[idx] = self.gt_sampling(config=cur_augmentor.sampler_cfg)
                return
    ...
    def forward(self, data_dict):
        ...
        for cur_augmentor in self.data_augmentor_queue:
            if isinstance(cur_augmentor, database_sampler.DataBaseSampler) and data_dict.get('disable_gt_sampling', False):
                continue
